...
CMD = '{snhic_path}/bin/randomize.loop.strength.py --condition-a {condition_a} --condition-b {condition_b} --sample-size-a {sample_size_a} --sample-size-b {sample_size_b} --sample-ids {sample_ids} --output-file {output_file} --genome-path {genome_path} --loops-file {loops_file} --cooler-dir {cooler_dir} --resolution {resolution} --threads {threads} --iterations {iterations}'
...
def main(samples_file, output_dir, tags, conditions, cooler_dir, cooler_ext, resolution, threads, iterations, genome_path, loops_file, snhic_path):
    ...
    for condition_pair in condition_pairs:
        ...
        cmd = CMD.format(snhic_path = snhic_path, condition_a = condition_a, condition_b = condition_b, sample_size_a = len(cellIDs_a), sample_size_b = len(cellIDs_b), sample_ids = ','.join([str(i) for i in cellIDs]), output_file = output_file, genome_path = genome_path, loops_file = loops_file, cooler_dir = cooler_dir, resolution = resolution, threads = threads, iterations = iterations)
        cmds.append(cmd)
